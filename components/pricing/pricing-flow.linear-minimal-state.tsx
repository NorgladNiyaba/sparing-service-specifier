...
function ProgressBar({ step }: { step: Step }) {
  ...
  return (
    <div className="rounded-[1.25rem] border border-[var(--brand-line)] bg-[var(--brand-panel)] px-4 py-4 shadow-[0_6px_20px_rgba(16,24,40,0.04)] sm:px-5">
      <div className="mb-4 flex items-center justify-between gap-4">
        <div className="text-[0.72rem] font-semibold uppercase tracking-[0.22em] text-[var(--brand-red)]">
          Guided Intake
        </div>
        <div className="rounded-full border border-[var(--brand-line)] bg-[var(--brand-panel-muted)] px-3 py-1 text-xs font-medium text-[var(--brand-muted)]">
          ...
          return (
            <div key={item.id} className="min-w-0 flex-1">
              <div className="flex items-center gap-3">
                <span
                  className={`flex h-7 w-7 items-center justify-center rounded-[0.7rem] border text-[0.68rem] font-semibold transition ${
                    active
...
function StepFrame({
  ...
  return (
    <section className="rounded-[1.75rem] border border-[var(--brand-line)] bg-[var(--brand-panel)] px-6 py-8 shadow-[0_10px_32px_rgba(16,24,40,0.04)] sm:px-10 sm:py-10">
...
function SentenceRow({
  ...
  return (
    <div className="space-y-3">
      <div className="max-w-3xl text-[1.3rem] leading-[1.72] text-[var(--brand-ink)] sm:text-[1.72rem] sm:leading-[1.62]">
        {children}
      </div>
...
function InlineTextInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`mx-2 inline-flex h-13 min-w-[8rem] rounded-[0.95rem] border border-[var(--brand-line)] bg-[var(--brand-panel-muted)] px-4 align-middle text-[1rem] font-medium text-[var(--brand-ink)] shadow-[0_2px_8px_rgba(16,24,40,0.03)] transition duration-200 focus:border-[var(--brand-red)] focus:bg-white focus:ring-4 focus:ring-[rgba(214,27,23,0.06)] hover:border-[var(--brand-line-strong)] sm:h-14 sm:text-[1.05rem] ${props.className ?? ""}`}
    />
  );
}
...
function InlineSelect(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      {...props}
      className={`mx-2 inline-flex h-13 min-w-[10rem] rounded-[0.95rem] border border-[var(--brand-line)] bg-[var(--brand-panel-muted)] px-4 align-middle text-[1rem] font-medium text-[var(--brand-ink)] shadow-[0_2px_8px_rgba(16,24,40,0.03)] transition duration-200 focus:border-[var(--brand-red)] focus:bg-white focus:ring-4 focus:ring-[rgba(214,27,23,0.06)] hover:border-[var(--brand-line-strong)] sm:h-14 sm:text-[1.05rem] ${props.className ?? ""}`}
    />
  );
}
    ...
    <div className="flex flex-wrap gap-3 pt-2">
      {values.map((value) => (
        <button
          ...
          className={`rounded-[0.95rem] border px-4 py-3 text-sm font-medium transition duration-200 ${
            selectedValue === value
              ? "border-[rgba(214,27,23,0.18)] bg-[var(--brand-red-soft)] text-[var(--brand-red)]"
              : "border-[var(--brand-line)] bg-[var(--brand-panel)] text-[var(--brand-muted)] hover:border-[var(--brand-line-strong)] hover:bg-[var(--brand-panel-muted)] hover:text-[var(--brand-ink)]"
          }`}
        >
          {formatter(value)}
        </button>
...
function PrimaryButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      {...props}
      className={`inline-flex h-12 items-center justify-center rounded-[0.95rem] bg-[var(--brand-red)] px-6 text-sm font-semibold text-white transition duration-200 hover:bg-[var(--brand-red-deep)] disabled:cursor-not-allowed disabled:opacity-50 ${props.className ?? ""}`}
    />
  );
}
...
function SecondaryButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      {...props}
      className={`inline-flex h-12 items-center justify-center rounded-[0.95rem] border border-[var(--brand-line)] bg-[var(--brand-panel)] px-6 text-sm font-semibold text-[var(--brand-ink)] transition duration-200 hover:border-[var(--brand-line-strong)] hover:bg-[var(--brand-panel-muted)] ${props.className ?? ""}`}
    />
  );
}
    ...
    <div className="min-h-screen bg-[var(--page-bg)]">
      <header className="border-b border-[var(--brand-line)]">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-6 px-5 py-5 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <div className="rounded-[1rem] bg-[var(--brand-red)] px-4 py-3">
      ...
      <main className="mx-auto max-w-5xl px-5 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
        ...
        <div className="mt-5 inline-flex max-w-full flex-wrap rounded-[0.95rem] border border-[var(--brand-line)] bg-[var(--brand-panel-muted)] px-4 py-2 text-sm leading-7 text-[var(--brand-muted)]">
          {continuityLabel}
        </div>
          ...
          <StepTransition stepKey={step}>
            ...
            {step === "recommendation" ? (
              ...
              >
                ...
                <div className="grid gap-4 border-t border-[var(--brand-line)] pt-8 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,22rem)]">
