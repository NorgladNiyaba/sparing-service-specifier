...
function ProgressBar({ step }: { step: Step }) {
  ...
  return (
    <div className="rounded-[1.5rem] border border-[var(--brand-line)] bg-[var(--brand-panel)] px-5 py-5 shadow-[0_4px_14px_rgba(16,24,40,0.03)] sm:px-6">
      ...
      <div className="relative mb-5 h-px w-full bg-[var(--brand-line)]">
        <div
          ...
          return (
            <div key={item.id} className="min-w-0 flex-1">
              <div className="flex items-center gap-3">
                <span
                  className={`flex h-7 w-7 items-center justify-center rounded-[0.85rem] border text-[0.68rem] font-semibold transition ${
                    active
...
function StepFrame({
  ...
  return (
    <section className="rounded-[2rem] border border-[var(--brand-line)] bg-[var(--brand-panel)] px-7 py-9 shadow-[0_8px_24px_rgba(16,24,40,0.03)] sm:px-12 sm:py-12">
      <div className="text-[0.72rem] font-semibold uppercase tracking-[0.24em] text-[var(--brand-red)]">
        {eyebrow}
      </div>
      <h1 className="mt-5 max-w-3xl text-balance text-[clamp(2.5rem,5vw,4.4rem)] leading-[1.02] tracking-[-0.055em] text-[var(--brand-ink)]">
        {title}
      </h1>
      {note ? <p className="mt-5 max-w-2xl text-base leading-8 text-[var(--brand-muted)]">{note}</p> : null}
      <div className="mt-12 space-y-8 sm:mt-14">{children}</div>
    </section>
  );
}
...
function SentenceRow({
  ...
  return (
    <div className="space-y-3">
      <div className="max-w-3xl text-[1.32rem] leading-[1.82] text-[var(--brand-ink)] sm:text-[1.72rem] sm:leading-[1.75]">
        {children}
      </div>
...
function InlineTextInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`mx-2 inline-flex h-14 min-w-[8rem] rounded-[1.05rem] border border-[var(--brand-line)] bg-[var(--brand-panel-muted)] px-5 align-middle text-[1rem] font-medium text-[var(--brand-ink)] shadow-[0_1px_4px_rgba(16,24,40,0.02)] transition duration-200 focus:border-[var(--brand-red)] focus:bg-white focus:ring-4 focus:ring-[rgba(214,27,23,0.05)] hover:border-[var(--brand-line-strong)] sm:text-[1.05rem] ${props.className ?? ""}`}
    />
  );
}
...
function InlineSelect(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      {...props}
      className={`mx-2 inline-flex h-14 min-w-[10rem] rounded-[1.05rem] border border-[var(--brand-line)] bg-[var(--brand-panel-muted)] px-5 align-middle text-[1rem] font-medium text-[var(--brand-ink)] shadow-[0_1px_4px_rgba(16,24,40,0.02)] transition duration-200 focus:border-[var(--brand-red)] focus:bg-white focus:ring-4 focus:ring-[rgba(214,27,23,0.05)] hover:border-[var(--brand-line-strong)] sm:text-[1.05rem] ${props.className ?? ""}`}
    />
  );
}
    ...
    <div className="flex flex-wrap gap-3 pt-3">
      {values.map((value) => (
        <button
          ...
          className={`rounded-[1rem] border px-4 py-3 text-sm font-medium transition duration-200 ${
            selectedValue === value
...
function PrimaryButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      {...props}
      className={`inline-flex h-12 items-center justify-center rounded-[1rem] bg-[var(--brand-red)] px-6 text-sm font-semibold text-white transition duration-200 hover:bg-[var(--brand-red-deep)] disabled:cursor-not-allowed disabled:opacity-50 ${props.className ?? ""}`}
    />
  );
}
...
function SecondaryButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      {...props}
      className={`inline-flex h-12 items-center justify-center rounded-[1rem] border border-[var(--brand-line)] bg-[var(--brand-panel)] px-6 text-sm font-semibold text-[var(--brand-ink)] transition duration-200 hover:border-[var(--brand-line-strong)] hover:bg-[var(--brand-panel-muted)] ${props.className ?? ""}`}
    />
  );
}
    ...
    <div className="min-h-screen bg-[var(--page-bg)]">
      <header className="border-b border-[var(--brand-line)]">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-6 px-5 py-5 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <div className="rounded-[1.15rem] bg-[var(--brand-red)] px-4 py-3">
