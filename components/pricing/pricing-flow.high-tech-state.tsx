...
function ProgressBar({ step }: { step: Step }) {
  ...
  return (
    <div className="rounded-[1.75rem] border border-[var(--brand-line)] bg-white px-4 py-4 shadow-[0_16px_50px_rgba(23,23,23,0.035)] sm:px-5">
      <div className="mb-4 flex items-center justify-between gap-4">
        <div className="text-[0.72rem] font-semibold uppercase tracking-[0.22em] text-[var(--brand-red)]">
          Guided Intake
        </div>
        <div className="rounded-full border border-[var(--brand-line)] bg-[var(--brand-surface)] px-3 py-1 text-xs font-medium text-[var(--brand-muted)]">
          {currentIndex + 1} / {steps.length}
        </div>
      </div>
      <div className="relative mb-4 h-px w-full bg-[var(--brand-line)]">
        <div
          className="absolute left-0 top-0 h-px bg-[linear-gradient(90deg,var(--brand-red),rgba(214,27,23,0.45))] transition-all duration-500"
          ...
          return (
            <div key={item.id} className="min-w-0 flex-1">
              <div className="flex items-center gap-3">
                <span
                  className={`flex h-7 w-7 items-center justify-center rounded-full border text-[0.68rem] font-semibold transition ${
                    active
                      ? "border-[var(--brand-red)] bg-[var(--brand-red)] text-white shadow-[0_10px_24px_rgba(214,27,23,0.18)]"
                      : complete
                        ? "border-[rgba(214,27,23,0.16)] bg-[var(--brand-red-soft)] text-[var(--brand-red)]"
...
function StepFrame({
  ...
  return (
    <section className="rounded-[2.2rem] border border-[var(--brand-line)] bg-white px-6 py-8 shadow-[0_24px_70px_rgba(23,23,23,0.04)] sm:px-10 sm:py-10">
...
function SentenceRow({
  ...
  return (
    <div className="space-y-3">
      <div className="max-w-3xl text-[1.35rem] leading-[1.75] text-[var(--brand-ink)] sm:text-[1.82rem] sm:leading-[1.65]">
        {children}
      </div>
...
function InlineTextInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`mx-2 inline-flex h-13 min-w-[8rem] rounded-full border border-[var(--brand-line)] bg-[linear-gradient(180deg,#ffffff,#fcfcfd)] px-5 align-middle text-[1rem] font-medium text-[var(--brand-ink)] shadow-[0_10px_28px_rgba(23,23,23,0.04)] transition duration-200 focus:border-[var(--brand-red)] focus:ring-4 focus:ring-[rgba(214,27,23,0.07)] hover:-translate-y-0.5 hover:border-[var(--brand-line-strong)] hover:shadow-[0_14px_34px_rgba(23,23,23,0.06)] sm:h-14 sm:text-[1.1rem] ${props.className ?? ""}`}
    />
  );
}
...
function InlineSelect(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      {...props}
      className={`mx-2 inline-flex h-13 min-w-[10rem] rounded-full border border-[var(--brand-line)] bg-[linear-gradient(180deg,#ffffff,#fcfcfd)] px-5 align-middle text-[1rem] font-medium text-[var(--brand-ink)] shadow-[0_10px_28px_rgba(23,23,23,0.04)] transition duration-200 focus:border-[var(--brand-red)] focus:ring-4 focus:ring-[rgba(214,27,23,0.07)] hover:-translate-y-0.5 hover:border-[var(--brand-line-strong)] hover:shadow-[0_14px_34px_rgba(23,23,23,0.06)] sm:h-14 sm:text-[1.1rem] ${props.className ?? ""}`}
    />
  );
}
    ...
    <div className="flex flex-wrap gap-3 pt-2">
      {values.map((value) => (
        <button
          ...
          className={`rounded-[1.1rem] border px-4 py-3 text-sm font-medium transition duration-200 ${
            selectedValue === value
              ? "border-[rgba(214,27,23,0.22)] bg-[linear-gradient(180deg,#fff5f4,#fbe8e7)] text-[var(--brand-red)] shadow-[0_14px_32px_rgba(214,27,23,0.08)]"
              : "border-[var(--brand-line)] bg-white text-[var(--brand-muted)] shadow-[0_8px_20px_rgba(23,23,23,0.03)] hover:-translate-y-0.5 hover:border-[var(--brand-line-strong)] hover:text-[var(--brand-ink)] hover:shadow-[0_14px_28px_rgba(23,23,23,0.06)]"
          }`}
        >
          {formatter(value)}
        </button>
...
function PrimaryButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      {...props}
      className={`inline-flex h-12 items-center justify-center rounded-full bg-[var(--brand-red)] px-6 text-sm font-semibold text-white shadow-[0_16px_36px_rgba(214,27,23,0.18)] transition duration-200 hover:-translate-y-0.5 hover:bg-[var(--brand-red-deep)] hover:shadow-[0_20px_40px_rgba(214,27,23,0.24)] disabled:cursor-not-allowed disabled:opacity-50 ${props.className ?? ""}`}
    />
  );
}
...
function SecondaryButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      {...props}
      className={`inline-flex h-12 items-center justify-center rounded-full border border-[var(--brand-line)] bg-white px-6 text-sm font-semibold text-[var(--brand-ink)] shadow-[0_10px_26px_rgba(23,23,23,0.03)] transition duration-200 hover:-translate-y-0.5 hover:border-[var(--brand-line-strong)] hover:bg-[var(--brand-surface)] hover:shadow-[0_14px_28px_rgba(23,23,23,0.05)] ${props.className ?? ""}`}
    />
  );
}
    ...
    <div className="min-h-screen bg-[var(--page-bg)]">
      <header className="border-b border-[var(--brand-line)]">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-6 px-5 py-5 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-[var(--brand-red)] px-4 py-3 shadow-[0_14px_32px_rgba(214,27,23,0.15)]">
      ...
      <main className="mx-auto max-w-5xl px-5 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
        ...
        <div className="mt-5 inline-flex max-w-full flex-wrap rounded-[1.2rem] border border-[var(--brand-line)] bg-white px-4 py-2 text-sm leading-7 text-[var(--brand-muted)] shadow-[0_8px_24px_rgba(23,23,23,0.03)]">
          {continuityLabel}
        </div>
          ...
          <StepTransition stepKey={step}>
            ...
            {step === "recommendation" ? (
              ...
              >
                ...
                <div className="grid gap-6 border-t border-[var(--brand-line)] pt-8 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,22rem)]">
                  <div className="rounded-[1.8rem] border border-[var(--brand-line)] bg-[linear-gradient(180deg,#ffffff,#fafafa)] p-6 shadow-[0_18px_48px_rgba(23,23,23,0.04)]">
